...
var checkUploadChunk = function (session, len) {
    if (!session || typeof(session.currentUploadSize) !== 'number' ||
            typeof(session.pendingUploadSize) !== 'number') {
        return 'NOT_READY';
    }
    if (!Number.isFinite(session.currentUploadSize) ||
            !Number.isFinite(session.pendingUploadSize) ||
            !Number.isSafeInteger(session.currentUploadSize) ||
            !Number.isSafeInteger(session.pendingUploadSize) ||
            session.currentUploadSize < 0 || session.pendingUploadSize <= 0) {
        return 'NOT_READY';
    }
    if (!Number.isSafeInteger(len) || len <= 0) {
        return 'INVALID_CHUNK';
    }
    if (session.currentUploadSize + len > session.pendingUploadSize) {
        return 'E_OVER_LIMIT';
    }
};

var validateStageSize = function (Env, safeKey, cb) {
    var session = Env.getSession(safeKey);
    if (!session || typeof(session.pendingUploadSize) !== 'number' ||
            !Number.isFinite(session.pendingUploadSize) ||
            !Number.isSafeInteger(session.pendingUploadSize) ||
            session.pendingUploadSize <= 0) {
        return void cb('NOT_READY');
    }
    Fs.stat(makeStagePath(Env, safeKey), function (err, stats) {
        if (err) { return void cb(err.code || err); }
        if (!stats || stats.size !== session.pendingUploadSize) {
            return void cb('E_SIZE_MISMATCH');
        }
        cb(void 0, stats.size);
    });
};
...
var uploadWs = function (Env, safeKey, content, cb) {
    ...
    var len = dec.length;
    var session = Env.getSession(safeKey);
    var ready = checkUploadChunk(session, len);
    if (ready) { return void cb(ready); }
...
var upload = function (Env, safeKey, content, cb) {
    ...
    var session = Env.getSession(safeKey);
    var ready = checkUploadChunk(session, dec.length);
    if (ready) { return void cb(ready); }

    var path = makeStagePath(Env, safeKey);
    Fs.appendFile(path, dec, function (err) {
        if (err) { return void cb(err); }
        session.currentUploadSize += dec.length;
        cb(void 0, dec.length);
    });
};
...
var upload_complete = function (Env, safeKey, id, cb) {
    ...
    nThen(function (w) {
        validateStageSize(Env, safeKey, w(function (e) {
            if (e) {
                w.abort();
                return void cb(e);
            }
