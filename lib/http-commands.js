...
var SENSITIVE_LOG_KEY = /secret|seed|mnemonic|password|token|bearer|jwt|auth|code|otp|recovery|contact|sig|signature/i;
var redactForLog = function (value, depth) {
    depth = depth || 0;
    if (depth > 6) { return '[REDACTED_DEPTH]'; }
    if (Array.isArray(value)) {
        return value.map(function (v) { return redactForLog(v, depth + 1); });
    }
    ...
    return Object.keys(value).reduce(function (acc, key) {
        ...
        acc[key] = redactForLog(value[key], depth + 1);
        return acc;
    }, {});
};
...
var handleResponse = function (Env, req, res) {
    ...
    if (Object.keys(body).some(k => !/(sig|txid)/.test(k))) {
        Env.Log.error("CHALLENGE_RESPONSE_DEBUGGING", redactForLog(body));
    ...
    if (!txid || typeof(txid) !== 'string' || txid.length !== 32) {
        Env.Log.error('CHALLENGE_RESPONSE_BAD_TXID', redactForLog(body));
        return void res.status(500).json({
            error: "Invalid txid",
        });
    }
    ...
    if (!sig || typeof(sig) !== 'string' || sig.length !== 88) {
        Env.Log.error("CHALLENGE_RESPONSE_BAD_SIG", redactForLog(body));
        return void res.status(500).json({
