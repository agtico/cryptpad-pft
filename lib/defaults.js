...
var getCspOrigin = function (value) {
    if (!value) { return ''; }
    try {
...
var getPostFiatConnectSrc = function (Env) {
    var nostr = Env.postFiat && Env.postFiat.nostr || {};
    var relays = []
        .concat(nostr.relays || [])
        .concat(nostr.privateRelays || [])
        .concat(nostr.relayProxy || []);
    return Array.from(new Set(relays.map(getCspOrigin).filter(Boolean))).join(' ');
};
...
Default.commonCSP = function (Env) {
    ...
    var postFiatConnectSrc = getPostFiatConnectSrc(Env);
    ...
    return [
        ...
        /*  this allows connections over secure or insecure websockets
            ...
            the ws://* directive
         */
        `connect-src 'self' blob: ${domain} ${sandbox} ${accounts_api} ${wsURL} ${postFiatConnectSrc}`,
