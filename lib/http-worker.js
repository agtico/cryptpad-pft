...
var servePostFiatAccountTx = async function (req, res) {
    ...
    if (marker) { rpcPayload.params[0].marker = marker; }
    ...
    for (var i = 0; i < rpcUrls.length; i += 1) {
        ...
        try {
            ...
            body.rpc_url = rpcUrl;
            return void sendPostFiatJson(res, body, 200);
        } catch (err) {
    ...
    sendPostFiatJson(res, { error: 'PFTL account_tx failed', detail: lastError }, 502);
};
