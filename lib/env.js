...
var normalizeNetworkId = function (value, fallback) {
...
var normalizeNostrRelays = function (value) {
    return normalizeStringList(value).filter(function (relay) {
        return /^wss?:\/\//i.test(relay);
    });
};
...
*/
        ...
        postFiat: {
            walletFirst: config.postFiat?.walletFirst !== false,
            disableLegacyLogin: config.postFiat?.disableLegacyLogin === true,
            pftl: {
                networkId: normalizeNetworkId(config.postFiat?.pftl?.networkId, 2025),
                rpcUrl: normalizeOptionalString(config.postFiat?.pftl?.rpcUrl),
                wssUrl: normalizeOptionalString(config.postFiat?.pftl?.wssUrl),
                ipfsGateway: normalizeOptionalString(config.postFiat?.pftl?.ipfsGateway),
                pointerMemoVersion: 'v4',
            },
            nostr: {
                relays: normalizeNostrRelays(config.postFiat?.nostr?.relays),
                privateRelays: normalizeNostrRelays(config.postFiat?.nostr?.privateRelays),
                relayProxy: normalizeOptionalString(config.postFiat?.nostr?.relayProxy),
